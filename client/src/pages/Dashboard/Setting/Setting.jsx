...
import { Edit, LogOut, X, Lock, Eye, EyeOff } from "lucide-react";
...
export default function Settings() {
  ...
  const handlePasswordSave = async () => {
    ...
    try {
      ...
      const verifyRes = await axios.post("/user/signin", {
        email: user.email,
  ...
  return (
    <div className="min-h-screen text-foreground flex items-center justify-center p-4">
      <motion.div
        ...
        className="w-full max-w-4xl bg-card p-8 border border-border rounded-xl"
      >
        {/* Profile Header */}
          ...
          <div className="relative mb-4">
            {user.role == "admin" ? (
              <img
                src={profilepic}
                alt=""
                className="w-32 h-32 rounded-full border-muted object-cover"
              />
            ) : (
              <div className="w-32 h-32 rounded-full text-white bg-accent flex items-center justify-center text-5xl font-black shadow-md select-none">
                {initials}
              </div>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 text-sm mb-8">
          ...
          />
          <InfoCard label="UUID" value={user.uuid} />
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 mt-10">
          <button
            onClick={handleEdit}
            className="bg-accent/60 text-foreground px-6 py-3 rounded-lg flex items-center gap-2 hover:bg-accent transition-all duration-300 shadow-sm"
          >
            <Edit className="w-4 h-4" /> Edit Profile
          </button>
          <button
            onClick={handleChangePassword}
            className="bg-primary/80 text-primary-foreground px-6 py-3 rounded-lg flex items-center gap-2 hover:bg-primary transition-all duration-300 shadow-sm"
          >
            <Lock className="w-4 h-4" /> Change Password
          </button>
          <button
            onClick={handleLogout}
            className="bg-destructive/70 text-white px-6 py-3 rounded-lg flex items-center gap-2 hover:bg-destructive transition-all duration-300 shadow-sm"
          >
      ...
      <AnimatePresence>
        {isChangingPassword && (
          <Modal onClose={handleClosePasswordModal}>
            <h3 className="text-xl font-semibold mb-4 text-center font-bungee">
              Change Password
            </h3>
...
const InfoCard = ({ label, value }) => (
  <div className="bg-muted/50 p-4 rounded-xl border border-border flex flex-col justify-between shadow-sm hover:shadow transition-all">
    ...
    <span className="text-base font-semibold mt-1 break-all">{value}</span>
  </div>
);
  ...
  <div>
    ...
    <div className="relative">
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full border border-border rounded-md bg-input px-3 py-2 focus:ring-2 focus:ring-ring outline-none pr-10"
      />
      <button
        type="button"
...
const Modal = ({ children, onClose }) => (
  ...
  >
    <motion.div
      ...
      className="bg-card w-full max-w-md p-6 rounded-xl shadow-xl border border-border relative mx-4"
    >
      <button
        onClick={onClose}
