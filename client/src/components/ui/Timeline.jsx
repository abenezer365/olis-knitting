...
export const Timeline = ({ data }) => {
  ...
  const heightTransform = useTransform(
    scrollYProgress,
    [0, 0.95], // Allow some buffer at the end
    [0, height]
  );
  ...
  const opacityTransform = useTransform(
    scrollYProgress,
    [0, 0.1, 0.9, 1],
    [0, 1, 1, 1] // Keep opacity at 1 for most of the scroll
  );
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-20 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-neutral-300 dark:bg-neutral-700 flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-600 dark:bg-neutral-400 border border-neutral-300 dark:border-neutral-600 p-2" />
              </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-0.5 bg-linear-to-t from-yellow-500 via-yellow-500 to-transparent from-0% via-10% rounded-full"
          />
        </div>
      </div>
    </div>
